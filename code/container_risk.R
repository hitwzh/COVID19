#remotes::install_github("joachim-gassen/tidycovid19", INSTALL_opts = c("--no-multiarch"))
...
hifleet_containers=fread("D:/06data/ships_hifleet/hifleetships_brief_container_with_mmsi.csv")
#提取原始数据，该数据以船舶stop的形式与相应的港口和国家进行了关联
filepaths=dir('D:/data/2020_container_stops_ports_country/',full.names = TRUE)
mmsis=data.table(dir('D:/data/2020_container_stops_ports_country/'))[,mmsi:=tstrsplit(V1,'.',fixed = TRUE)[1]]$mmsi 
...
#百万人口增长率
merged_dta=merged_dta0[order(iso3c,date)][,newadd:=(confirmed-lag(confirmed))][,iso3c1:=lag(iso3c)]
...
for(k in seq(1,shipn)){
  ...
  if(n>0){
    ...
    if(nrow(res)>0){
      ...
      fwrite(res1,paste('D:/data/2020container/shipriskindex/',ammsi,'.csv',sep = ''))
    }
  }
...
filepaths=dir('D:/data/2020container/shipriskindex/',full.names = TRUE)
mmsis=data.table(dir('D:/data/2020container/shipriskindex/'))[,mmsi:=tstrsplit(V1,'.',fixed = TRUE)[1]]$mmsi 
...
fwrite(port14Index,'D:/data/2020container/port14index/port14index.csv')
port14Index=fread('D:/data/2020container/port14index/port14index.csv')
aportindex1=port14Index[country=='China'][portname=='Zhoushan']
aportindex1=aportindex1[,rmean_7_sd_index:=rollmean(sd_14_index, 7, na.pad=TRUE, align="right")]
plot(aportindex1$date,aportindex1$acm_14_acIndex,type = "l")
plot(aportindex1$date,aportindex1$rmean_7_sd_index,type = "l")

plot(res1$date,res1$acm_14_infectedIndex,'l')
plot(res1$date,res1$acm_14_densityIndex,'l')
plot(res1$date,res1$acm_14_densityIndex)
plot(res1$date,res1$acm_14_addIndex,'l')
plot(res1$date,res1$acm_14_addIndex)

port14Index[acm_14_Index>0]

#create network

filepaths=dir('D:/data/cruiseships/shipriskindex/',full.names = TRUE)
astopship=fread(filepaths[1])[,list(date,starttime,portid,portname,iso3c,country)][portid<0]
for(i in seq(1,length(filepaths))){
  print(i)
  af=fread(filepaths[i])[,list(date,starttime,portid,portname,iso3c,country)]
  astopship=rbind(astopship,af)
}
#astopship=fread(filepaths[1])[,list(date,starttime,portid,portname,iso3c,country)]
astopship[,portid2:=shift(portid,-1)][,portname2:=shift(portname,-1)][,iso3c2:=shift(iso3c,-1)][,country2:=shift(country,-1)]
portlinks0=astopship[!is.na(portid2)&(portid!=portid2),.N,list(portid,portid2,portname,portname2,iso3c,iso3c2,country,country2)]
hifleetports_sf0=fread('D:/Git/Rprojects/basedata/hifleetports.csv',encoding = 'UTF-8')
portlinks1=data.table(left_join(portlinks0,hifleetports_sf0[,list(id,lon,lat)],by=c('portid'='id')))
portlinks2=data.table(left_join(portlinks1,hifleetports_sf0[,list(id,lon2=lon,lat2=lat)],by=c('portid2'='id')))
port14index0=fread('D:/data/cruiseships/port14index/port14index.csv')
port14index=data.table(left_join(port14index0,hifleetports_sf0[,list(id,lon,lat)],by=c('portid'='id')))
fwrite(port14index,'D:/data/cruiseships/port14index/20201018port14index.csv')
fwrite(portlinks2,'D:/data/cruiseships/portnetwork/20201018portlinks.csv')
